...
public class ReUtil
{
    ...
    /**
     * 从content中匹配出多个值并根据template生成新的字符串<br> 例如：<br> content 2013年5月 pattern
     * (.*?})年(.*?)月 template： $1-$2 return 2013-5
     *
     * @param pattern
     *            匹配正则
     * @param content
    ...
    /**
     * 从content中匹配出多个值并根据template生成新的字符串<br> 匹配结束后会删除匹配内容之前的内容（包括匹配内容）<br>
     * 例如：<br> content 2013年5月 pattern (.*?})年(.*?)月 template： $1-$2 return
     * 2013-5
     *
     * @param pattern
     *            匹配正则
