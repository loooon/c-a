...
public class XssUtil
{
    public static String escape(String s)
    {
        ...
        for (int i = 0; i < s.length(); i++)
        {
            ...
            case '\\':
                ...
                break;
            /*
             * case '&': sb.append('&');// 全角 break; case '#': sb.append('＃');//
             * 全角井号 break; case ':': sb.append('：'); // 全角冒号 break;
             */
            case '%':
                sb.append('％'); // 全角冒号
                break;
    ...
    {
        /*
         * if (s == null || s.isEmpty()) { return s; } StringReader reader = new
         * StringReader( s ); StringWriter writer = new StringWriter(); try {
         * HTMLParser.process(reader, writer, new XSSFilter(), true ); String
         * result = writer.toString(); return result; } catch
         * (NullPointerException e) { return escape(s); } catch(Exception ex) {
         * ex.printStackTrace(); } finally { try { reader.close();
         * writer.close(); } catch(Exception ce) { ce.printStackTrace(); } }
         * return null;
         */
        if (s == null || s.isEmpty())
        {
