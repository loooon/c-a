...
//                data: param,
        ...
        $("#userTable").on('click','[name=updateUserBaseInfo]',function ()
        {
            ...
            $("[name=userName]").val(full_obj.username);
            $("[name=companyName]").val(full_obj.companyname);
            $("[name=productName]").val(full_obj.productname);
            $("[name=phone]").val(full_obj.phone);
            $("[name=email]").val(full_obj.email);
//            $("[name=expireDate]").val(full_obj.expiredate);
            $("[name=expireDate]").val( new Date(full_obj.expiredate).format("MM/dd/yyyy"));
...
//            debug:true,
            rules: {
                userName: {
                    required:true,
                    remote:{
                        url:'${basePath}/user/nonentity/name',
                        data:{userId:function(){return $("[name=userId]").val();},
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function(data, type, full, meta) {
                        ...
                        return '<button style="margin-right:5px;" type="button" class="btn btn-primary btn-xs" name="updateUserBaseInfo" data-userName="'+full.userName+'" data-companyName="'+full.companyName+'" data-productName="'+full.productName+'" data-phone="'+full.phone+'" data-email="'+full.email+'" data-expiredate="'+full.expireDate+'" commodity-id="' + full.userId + '"><i class="fa fa-edit"></i>修改</button><button style="margin-right:5px;" type="button" class="btn btn-primary btn-xs" name="updateUserPermission" data-attr="'+full.vkey+'" commodity-id="' + full.userId + '"><i class="fa fa-edit"></i>修改服务</button>';
