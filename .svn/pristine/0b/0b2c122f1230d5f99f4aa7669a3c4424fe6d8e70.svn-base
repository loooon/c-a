...
/**
 * 验证PKCS7签名 注意：此类目前使用了jdk中sun.security包中的类， 根据Sun公司的声明: sun.* Packages *
 * 中的类可能有问题，且以后不一定兼容。 目前测试下来，Windows/Linux平台上的jdk1.3、1.4都正常，但不排除以后可能出问题的情况。
...
public class CertUtil
{
     ...
     * @throws Exception
     */
    public static void verfiySign(String b64SignedData, Certificate adminCert, String signTxt, Date signDate)
            throws Exception
    {
     ...
     * @throws Exception
     */
    public static void verfiySigneByPubKey(String b64SignedData, String b64Cert, String signTxt, Date signDate)
            throws Exception
    {
     ...
     * @throws Exception
     */
    public static void verfiySigneByPubKey(String b64SignedData, Certificate adminCert, String signTxt, Date signDate)
            throws Exception
    {
     ...
     * @throws Exception
     */
    public static void verfiySignEx(String b64SignedData, Certificate adminCert, String signTxt, Date signDate,
            boolean onlyVerifyPubKey) throws Exception
    {

        // 客户端负责删除回车符号
        /*
         * byte[] signedData = null; try { signedData =
         * Base64.decode(b64SignedData.getBytes()); } catch (Exception e) {
         * String err = "解析base64编码的签名数据失败，数据不是base64编码格式（可能数据被篡改）。原因：" +
         * e.getMessage(); logger.error(err, e); throw new Exception(err); } try
         * { PKCS7 p7 = new PKCS7(signedData); // 先验证证书是否一致 //
         * 虽然PKCS7规范中证书是可选的，但目前客户端通过CSP签名，其中总是包含证书 X509Certificate[] signerCerts
         * = p7.getCertificates(); if ((signerCerts == null) ||
         * (signerCerts.length < 1)) throw new
         * Exception("错误的PKCS7签名格式，缺少签名者的证书"); // 判断证书是否一致：遍历PKCS7中的证书，找到一致的为止
         * // 否则抛出异常。遍历是必须的，因为即使只有一个客户端签名， // 这里仍可能会有多张证书（相同的CAPICOM，有些客户端环境下 //
         * 会提交证书链，有些不会，原因未知） boolean bMatchCert = false; for (int i = 0; i <
         * signerCerts.length; i++) { // 如果onlyVerifyPubKey为真，仅比较证书公钥，否则比较整个证书
         * if (onlyVerifyPubKey) { if
         * (adminCert.getPublicKey().equals(signerCerts[i].getPublicKey())) {
         * bMatchCert = true; break; } } else { if
         * (adminCert.equals(signerCerts[i])) { bMatchCert = true; break; } } }
         * if (!bMatchCert) throw new Exception("签名用的证书与系统中对应的该用户的证书不一致"); //
         * 验证签名 SignerInfo[] singerInfos = p7.verify(); if ((singerInfos ==
         * null) || (singerInfos.length == 0)) { throw new
         * Exception("pkcs7 验证失败"); } // PKCS7数据验证成功，再验证签名数据是否是期望数据 String
         * signContent = new String(p7.getContentInfo().getContentBytes(),
         * "UTF-16LE"); if (!signTxt.equals(signContent)) { throw new
         * Exception("签名数据与期望数据不一致，可能数据已被篡改"); } // 如果需要签名时间，设置之 if (signDate !=
         * null) { Date d = (Date)
         * singerInfos[0].getAuthenticatedAttributes().getAttributeValue
         * ("SigningTime"); signDate.setTime(d.getTime()); } } catch (Exception
         * e) { String err = "签名验证失败，原因：" + e.getMessage(); logger.error(err,
         * e); throw new Exception(err, e); }
         */
    }
    ...
    public static void main(String[] args)
    {
        try
        {
            ...
            String b64Cert = "MIIF3TCCBMWgAwIBAgIHI1sg+sl85jANBgkqhkiG9w0BAQUFADBfMQswCQYDVQQG"
                    ...
                    + "yF2gS5YLwDgNo7Di9vmE1HvGtzTFAha+9G1VL37fybFJ0yRrp6BH7t80hJ+B6bFx" + "u8i4a0er/NAL0VANab+Pabg=";
